...
public class TrainingSessionController : ControllerBase
{
    ...
    public async Task<IActionResult> StartSession([FromBody] StartSessionRequest request)
    {
        ...
        var session = await _sessionService.StartSessionAsync(userId, request.ScenarioId, request.Mode, request.TeamId);
        await _auditService.LogAsync("StartSession", "TrainingSession", session.Id.ToString(), null, $"Scenario:{request.ScenarioId}", HttpContext);
        return CreatedAtAction(nameof(GetById), new { id = session.Id }, session);
    }

    [Authorize]
    [HttpPost("{sessionId}/decision")]
    public async Task<IActionResult> RecordDecision(Guid sessionId, [FromBody] RecordDecisionRequest request)
    {
        var decision = await _sessionService.RecordDecisionAsync(
            sessionId, request.ActionType, request.UserX, request.UserY,
            request.UserTiming, request.OptimalX, request.OptimalY,
            request.OptimalTiming, request.ActionData);
...
public record StartSessionRequest(Guid ScenarioId, TrainingMode Mode, Guid? TeamId);
public record RecordDecisionRequest(string ActionType, decimal UserX, decimal UserY, decimal UserTiming, decimal OptimalX, decimal OptimalY, decimal OptimalTiming, string? ActionData);
