...
export default function AdminCoupons() {
  ...
  const loadItems = async () => {
    setLoading(true);
    try {
      const [coupons, disc] = await Promise.all([api.discounts.listCoupons(), api.discounts.list()]);
      setItems(coupons);
      setDiscounts(disc);
    } catch {
  ...
  const openEdit = (item: Coupon) => { setEditingItem(item); setForm(item); setShowForm(true); };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem) {
        await api.discounts.updateCoupon(editingItem.id, form);
        setMessage("Coupon updated");
      } else {
        await api.discounts.createCoupon(form);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await api.discounts.deleteCoupon(id);
  ...
  const getDiscountName = (discountId: string) => discounts.find((d) => d.id === discountId)?.code || discountId;
    ...
    <div>
      ...
      {showForm && (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-6 mb-6">
          <h2 className="text-lg font-bold text-white mb-4">{editingItem ? "Edit Coupon" : "Create Coupon"}</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.discountId || ""} onChange={(e) => setForm({ ...form, discountId: e.target.value })} className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500">
                  <option value="">None</option>
                  {discounts.map((d) => (
                    <option key={d.id} value={d.id}>{d.code} ({d.percentage}% / ${d.fixedAmount})</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-gray-400 text-sm mb-1">Max Uses</label>
                <input type="number" value={form.maxUses || 0} onChange={(e) => setForm({ ...form, maxUses: Number(e.target.value) })} className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
                <label className="block text-gray-400 text-sm mb-1">Min Amount</label>
                <input type="number" value={form.minAmount || 0} onChange={(e) => setForm({ ...form, minAmount: Number(e.target.value) })} className="w-full bg-gray-700 text-white rounded-lg px-3 py-2 border border-gray-600 focus:outline-none focus:ring-2 focus:ring-green-500" />
              </div>
              <div>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-700">
                ...
                <th className="px-4 py-3 text-gray-400 text-sm font-medium">Max Uses</th>
                <th className="px-4 py-3 text-gray-400 text-sm font-medium">Current Uses</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="px-4 py-6 text-gray-400 text-center">No coupons found</td></tr>
              ) : items.map((item) => (
                <tr key={item.id} className="border-b border-gray-700/50 hover:bg-gray-700/30">
                  <td className="px-4 py-3 text-white text-sm font-medium">{item.code}</td>
                  <td className="px-4 py-3 text-gray-300 text-sm">{getDiscountName(item.discountId)}</td>
                  <td className="px-4 py-3 text-white text-sm">{item.maxUses}</td>
                  <td className="px-4 py-3 text-white text-sm">{item.currentUses}</td>
